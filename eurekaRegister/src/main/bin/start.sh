...
if [ -n "$PIDS" ]; then
    echo "ERROR: The already started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$DEPLOY_DIR/logs/std.log
...
echo -e "Starting the ...\c"
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS org.ligson.cloud.eureka.server.main.RegisterMain > $STDOUT_FILE 2>&1 &
